function [segment_pts, coms, polygons] = segmentTracking_WM(im, varargin)
...
%  average of these two.
    ...
    addOptional(p, 'last_orientation', defaultLastOrientation);
    parse(p, im, varargin{:});
    
    
    %% Find beginnig of tail using eyes
    
    % First COM (mean of points inside first circle)
    f_ind1 = find(p.Results.im <= quantile(p.Results.im(:), p.Results.num_pix1/numel(p.Results.im)));
    pix_value1 = (256 - double(p.Results.im(f_ind1)));
    [fx1, fy1] = ind2sub(size(p.Results.im), f_ind1);
    com1 = pix_value1' * [fx1, fy1] ./ sum(pix_value1);

    % Second COM (mean of points inbetween first and second circles)
    f_ind2 = find(p.Results.im <= quantile(p.Results.im(:), p.Results.num_pix2/numel(p.Results.im)));
    pix_value2 = (256 - double(p.Results.im(f_ind2)));
    [fx2, fy2] = ind2sub(size(p.Results.im), f_ind2);
    % Removing points that are too close to com1
    ok_radius = 7.5;
    ok_points = (((fx2 - com1(1)).^2 + (fy2 - com1(2)).^2) >= ok_radius.^2);
    pix_value2 = pix_value2(ok_points);
    fx2 = fx2(ok_points);
    fy2 = fy2(ok_points);
    % Computing center of mass 2
    com2 = pix_value2' * [fx2, fy2] ./ sum(pix_value2);
